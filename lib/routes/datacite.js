...
router.get('/search', (req, res, next) => {
  ...
  if (!isEmpty(jsonLdQuery)) {
    /*takes all the jsonLD query params and joins them to be searched*/ //eslint-disable-spaced-comment
    const searchFor = flatten(flatMap(jsonLdQuery)).join(' ');
    searchDatacite(searchFor).then((repos) => {
      /*repos.response.docs is where the repository specific metadata info is stored*/ //eslint-disable-spaced-comment
